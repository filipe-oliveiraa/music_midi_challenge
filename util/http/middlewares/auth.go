...
const (
	URLAuthPrefix    = "/urlAuth/:" + TokenPathParam
	urlAuthFormatter = "/urlAuth/%s"
)

// InvalidTokenMessage is the message set when an invalid / missing token is found.
const InvalidTokenMessage = "Invalid API Token" //nolint: gosec

// AuthMiddleware provides some data to the handler.
type AuthMiddleware struct {
	// Header is the token header which needs to be provided. For example 'X-Gorxestra-API-Token'.
	header string
